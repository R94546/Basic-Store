...
class _POSScreenState extends State<POSScreen> {
  ...
  void initState() {
    super.initState();
    // Global skaner ushlovchisi вЂ” faqat POS ekrani ochiq turganda faol
  ...
  /// fokusda bo'lsa вЂ” TextField o'zi ishlaydi (onSubmitted). Aks holda
  /// belgilarni buferga yig'ib, Enter kelganda mahsulotni qidiramiz.
  bool _onGlobalKey(KeyEvent event) {
    if (event is! KeyDownEvent) return false;
    // Qidiruv maydoni yoki boshqa matn maydoni fokusda bo'lsa вЂ” aralashmaymiz
    if (_searchFocusNode.hasFocus) return false;
    final primary = FocusManager.instance.primaryFocus;
    if (primary?.context?.widget is EditableText) return false;

    final now = DateTime.now();
    // Belgilar orasidagi tanaffus katta bo'lsa (odam yozyapti) вЂ” buferni tozalaymiz.
  ...
  Future<void> _onSearchSubmit(String value) async {
    ...
    final pp = context.read<ProductProvider>();
    // Shtrix yoki artikul (qisqa raqam) bo'yicha вЂ” darhol, xotiradan.
    ...
    if (hit != null) {
      ...
      // Shtrixsimon kod topilmadi вЂ” mahsulotga biriktirishni taklif qilamiz
  ...
  Future<void> _showBindBarcodeDialog(String code) async {
    ...
    // Shtrix tovarga umumiy biriktirildi. Variantli bo'lsa вЂ” savatga
    // tushirmasdan variantlarni ochamiz; oddiy bo'lsa darhol qo'shamiz.
    if (product.hasVariants) {
      await _onProductTap(product);
    } else {
      _addToCart(product);
    }
  ...
  Future<void> _checkout() async {
    ...
    final changeMsg = sale.changeAmount > 0
        ? ' вЂў ${loc.t('pos.change')}: ${_money(sale.changeAmount)}'
        : '';
        ...
        Expanded(
          child: Consumer<ProductProvider>(
            builder: (context, provider, _) {
              ...
              if (q.isNotEmpty) {
                // Nom + narx + shtrix + artikul bo'yicha ("Fut 200" kabi):
                // har bir so'z mos kelishi shart.
                final tokens = q.split(RegExp(r'\s+'));
                products = products.where((p) {
                  final hay = '${p.name} ${p.price} ${p.barcode} '
                          '${BarcodeUtil.articleOf(p.barcode)}'
                      .toLowerCase();
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text('${loc.t('pos.pay')}  вЂў  $_cartCount'),
                    style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.all(16),
      ...
      child: GlassCard(
        ...
        child: SizedBox(
          ...
          child: Column(
            ...
            children: [
              Text('${product.name} вЂ” ${loc.t('pos.chooseVariant')}',
                  style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                      color: AppTheme.textPrimary)),
...
class _CartItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sub = item.variant != null
        ? '${item.variant!.color} вЂў ${item.variant!.size}'
        : null;
    return Container(
...
class _PaymentDialogState extends State<_PaymentDialog> {
  ...
  Widget _keypad(LocaleProvider loc) {
    final keys = ['7', '8', '9', '4', '5', '6', '1', '2', '3', '000', '0', 'вЊ«'];
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Wrap(
